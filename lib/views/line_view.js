App.Views.LineView = Backbone.D3View.extend({
	tagName: "g",
	...
	remove: function(){
		d3.select(this.el).transition().duration(0);
		...
		var that = this,
			...
			timer = setInterval(function(){
				...
				.on("mouseenter", function(d){
					...
					d3.select(this).style("fill", that.model.get("color"));
				})
	...
	attachToolTip: function(options){
		var that = this;
		var g = d3.select(this.el).append('g').attr("class", "tool-tip");
		var defaults = {
		...
		this.tooltip = new App.Views.ToolTip(_.extend(options, defaults));
		this.tooltip.render();
	},
